...
async function getPage(idPage) {
  const result = await pool.query(
    `SELECT * FROM paginas WHERE id_pagina = ${idPage}`
  );
  let jsonResult = result.rows.map((row) => {
    return {
...
async function getPromotions(idItems) {
  const result =
    await pool.query(`select p.id_promocion,p.porcentaje,p.estado,p.nombre_promocion,p.descuento from articulos a
...
async function setCategoryItem(idCategory, idItem) {
  try {
    const result =
      await pool.query(`insert into categorias_articulos (id_articulo, id_categoria)
